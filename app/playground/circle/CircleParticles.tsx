...
interface ParticlesProps extends ComponentPropsWithoutRef<"div"> {
  className?: string;
  quantity?: number;   // particle count
  staticity?: number;  // how strongly particles are attracted to cursor
  ease?: number;       // easing of following the cursor
  size?: number;       // base size (added to random 0–2)
  refresh?: boolean;   // toggle to rebuild particles
  color?: string;      // hex color, e.g. "#ffffff"
  vx?: number;         // global x velocity drift
  vy?: number;         // global y velocity drift
}
...
export const Particles: React.FC<ParticlesProps> = ({
  ...
  }, [color]); // rebuild on color change to recalc rgb
  ...
  const resizeCanvas = () => {
    ...
    // reset transform and set DPR scaling
